-- Fix salary duplication: only use SALAIRES COACH in monthly_kpis
...
BEGIN
  ...
  -- Calculate expenses by actual category names in DB (ONLY SALAIRES COACH, not SALAIRES)
  SELECT 
  ...
  -- Calculate total expenses (only coach salaries)
  total_exp := expense_ad_spend + expense_rent + expense_software + expense_internet + 
               expense_subscriptions + expense_bank + expense_salaries_coach +
  ...
  VALUES (
    ...
    expense_subscriptions,
    expense_bank,
    expense_salaries_coach,  -- Only coach salaries
    expense_food,
    expense_credit,
    total_exp
  )
...
-- Re-sync existing expense data (ONLY SALAIRES COACH)
...
SELECT 
  ...
  SUM(CASE WHEN category = 'SALAIRES COACH' THEN amount ELSE 0 END) as salaries,
...
-- Recalculate profit after fixing salaries
