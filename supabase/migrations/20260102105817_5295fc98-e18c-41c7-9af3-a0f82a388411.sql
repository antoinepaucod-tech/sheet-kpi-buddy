...
-- Drop and recreate the sync trigger to properly handle weekly instructor changes
...
BEGIN
  -- Handle DELETE: remove accounting entries for instructors from this course
  IF TG_OP = 'DELETE' THEN
    -- Get all unique instructors from this month (excluding the deleted course)
    ...
    -- Delete orphaned accounting entries
  ...
  IF all_instructors IS NOT NULL THEN
    FOREACH instr IN ARRAY all_instructors LOOP
      -- Calculate total for this instructor across all courses in this month
      -- Each week where this instructor taught (and there was attendance) adds their hourly rate
      ...
      IF existing_transaction_id IS NOT NULL THEN
        ...
        ELSE
          -- Delete if total is 0 (instructor has no paid hours)
          DELETE FROM accounting_transactions WHERE id = existing_transaction_id;
        END IF;
      ELSE
        -- Only create entry if there's a positive amount
        IF instructor_total > 0 THEN
...
-- Also update calculate_course_kpis to correctly calculate monthly_expenses
-- It should sum the hourly rates of the ACTUAL instructor for each week
...
BEGIN
  ...
  -- Week 1
  week_instructor := COALESCE(NULLIF(NEW.week1_instructor, ''), NEW.instructor);
  ...
  -- Week 2
  week_instructor := COALESCE(NULLIF(NEW.week2_instructor, ''), NEW.instructor);
  ...
  -- Week 3
  week_instructor := COALESCE(NULLIF(NEW.week3_instructor, ''), NEW.instructor);
  ...
  -- Week 4
  week_instructor := COALESCE(NULLIF(NEW.week4_instructor, ''), NEW.instructor);
  ...
  -- Week 5
  week_instructor := COALESCE(NULLIF(NEW.week5_instructor, ''), NEW.instructor);
