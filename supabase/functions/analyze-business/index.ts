...
serve(async (req) => {
  ...
  try {
    ...
    const avgRevenue = recentMonths.reduce((sum: number, m: any) => sum + (m.total_revenue || 0), 0) / (recentMonths.length || 1);
    const avgExpenses = recentMonths.reduce((sum: number, m: any) => sum + (m.total_expenses || 0), 0) / (recentMonths.length || 1);
    const revenueGrowth = recentMonths.length >= 2 ? 
      ((recentMonths[recentMonths.length - 1]?.total_revenue - recentMonths[0]?.total_revenue) / (recentMonths[0]?.total_revenue || 1) * 100) : 0;
