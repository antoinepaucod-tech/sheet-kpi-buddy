  ...
  return (
    <div className="space-y-4" data-testid="kpi-detailed-view">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2">
        ...
        <StatBox label={lang === "fr" ? "ACRM Expected" : "ACRM Expected"} value={formatCHF(kpi.acrm_expected)} icon={DollarSign} color="text-[#FF9F0A]" tooltip={lang === "fr" ? "Revenu attendu des abonnements mensualisés / Membres actifs" : "Expected subscription revenue / Active members"} />
        <StatBox label="Collection Rate" value={`${(kpi.collection_rate ?? 0).toFixed(1)}%`} icon={TrendingUp} color={(kpi.collection_rate ?? 0) >= 100 ? "text-[var(--color-success)]" : "text-[var(--color-warning)]"} tooltip={lang === "fr" ? "Cash encaissé / Revenue attendu des souscriptions x 100" : "Cash collected / Expected revenue x 100"} />
